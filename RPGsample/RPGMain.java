...
public class RPGMain {
    ...
    private final int MONSTER_SUM_NUM=4; //現在存在するキャラクター数
    ...
    private int turn=1;
    private boolean battleEndFlag=true;
    ...
    public void game(){
        ...
        while(battleEndFlag){
            ...
            dispStatus();
            //入力されたコマンドを取得
            boolean commandErrorFlag = false;

            do{
                int command = sc.nextInt();
                commandErrorFlag = false;
                if( command == 1 || command == 2){
                    ...
                    commandErrorFlag = true;
                }
            }while(commandErrorFlag);
        }
        sc.close();
    }
    ...
    private void dicideMonsters(){
        ...
        for(int i=0; i < MONSTER_NUM; i++){
            //乱数を取得してモンスターを決定する
            int value = r.nextInt(MONSTER_SUM_NUM);
            if( value == 0 ){
